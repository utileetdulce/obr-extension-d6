...
class DiceFactory {
  ...
  createTextMaterial(
    ...
    let cachestring =
      diceobj.type + textCache + index + texture.name + forecolor + outlinecolor + backcolor
    if (diceobj.shape == "d4") {
  ...
  setMaterialInfo(colorset = "") {
    ...
    if (this.dice_material_rand == "" && Array.isArray(this.dice_material)) {
      this.dice_material_rand =
        this.dice_material[Math.floor(Math.random() * this.dice_material.length)]
